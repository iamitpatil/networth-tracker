...
export default function UpstoxSync({ onSyncComplete }) {
  ...
  const handleConnect = async () => {
    try {
      const { data } = await client.get('/brokers/upstox/auth-url')
  ...
  const handleSync = async () => {
    setSyncing(true)
    try {
      const { data } = await client.post('/brokers/upstox/sync')
      if (data.success) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium">
              {isConnected ? 'Upstox Connected' : isExpired ? 'Upstox Session Expired' : 'Import from Upstox'}
            </p>
            <p className="text-xs text-[var(--text-muted)]">
              ...
              {!status?.connected && 'Connect your Upstox account to auto-import holdings'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {(isExpired || !status?.connected) && (
            ...
            >
              ...
              {connecting ? 'Connecting...' : isExpired ? 'Reconnect' : 'Connect Upstox'}
            </button>
          )}
        </div>
      </div>
